...
impl Framebuffer {
    ...
    ///
    /// let mut display = console.get_display().unwrap();
    /// let framebuffer = display.get_framebuffer().unwrap();
    ...
    pub async fn get_image(&self, full_image: bool, timeout_milliseconds: i32) -> Result<FramebufferEvent, VboxError> {
        let start_time = std::time::Instant::now();
        let (response_tx, response_rx) = tokio::sync::oneshot::channel::<FramebufferEvent>();

        self.tx
            .send(FramebufferEventInternal::GetImage(response_tx, full_image))
            .await
            .map_err(|err| {
                VboxError::new(
                    0,
                    "get_image",
                    format!("Failed to send GetImage event to channel: {:?}", err),
                    None,
                )
            })?;
        debug!("waiting for image, elapsed: {:?}", start_time.elapsed().as_secs_f64());
        debug!("waiting for image");
        // Ожидаем отклик через одноразовый канал
        let response = tokio::time::timeout(
            std::time::Duration::from_millis(timeout_milliseconds as u64),
            response_rx,
        ).await;
        debug!("waiting for image, elapsed 1: {:?}", start_time.elapsed().as_secs_f64());
        match response {
            Ok(event) => {
                event.map_err(|err| {
                    VboxError::new(
                        0,
                        "get_image",
                        format!("Failed to receive GetImage event from channel: {:?}", err),
                        None,
                    )
                })
            }
            Err(_) => {
                Ok(FramebufferEvent::None)
            }
        }
    }
}
