...
use log::error;
use std::fmt::Display;
use vbox_raw::sys_lib::{IAdditionsStateChangedEvent, IEvent, IADDITIONSSTATECHANGEDEVENT_IID_STR};

/// Notification when a Guest Additions property changes.
#[derive(Debug)]
pub struct AdditionsStateChangedEvent {
    pub midl_does_not_like_empty_interfaces: bool,
}

impl AdditionsStateChangedEvent {
    pub fn new(object: *mut IEvent) -> DetailEvent {
        match Self::create(object) {
            Ok(detail) => DetailEvent::AdditionsStateChangedEvent(detail),
            Err(err) => {
                error!("AdditionsStateChangedEvent error:{}", err);
                DetailEvent::Null
            }
        }
    }
    fn create(object: *mut IEvent) -> Result<Self, VboxError> {
        let obj1 = cast_event(object, IADDITIONSSTATECHANGEDEVENT_IID_STR)?;
