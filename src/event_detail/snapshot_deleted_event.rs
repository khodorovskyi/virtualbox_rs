...
use log::error;
use std::fmt::Display;
use vbox_raw::sys_lib::{IEvent, ISnapshotDeletedEvent, ISNAPSHOTDELETEDEVENT_IID_STR};
...
impl SnapshotDeletedEvent {
    pub fn new(object: *mut IEvent) -> DetailEvent {
        match Self::create(object) {
            Ok(detail) => DetailEvent::SnapshotDeletedEvent(detail),
            Err(err) => {
                error!("SnapshotDeletedEvent error:{}", err);
                DetailEvent::Null
            }
        }
    }
    fn create(object: *mut IEvent) -> Result<Self, VboxError> {
        let obj1 = cast_event(object, ISNAPSHOTDELETEDEVENT_IID_STR)?;
    ...
    fn get_midl_does_not_like_empty_interfaces(
        new_obj: *mut ISnapshotDeletedEvent,
    ) -> Result<bool, VboxError> {
        get_function_result_bool!(new_obj, GetMidlDoesNotLikeEmptyInterfaces)
    }

    fn get_machine_id(object: *mut ISnapshotDeletedEvent) -> Result<&'static str, VboxError> {
        get_function_result_str!(object, GetMachineId)
    }
    fn get_snapshot_id(object: *mut ISnapshotDeletedEvent) -> Result<&'static str, VboxError> {
        get_function_result_str!(object, GetSnapshotId)
    }
}

impl Display for SnapshotDeletedEvent {
